# train_llm/train_matformer.py
# Orchestrator for MatFormer training; exports run_training_from_cfg for train_llm.py.
...
from model_implementations import load_tokenizer_with_fallback, load_model
from .train_dataset import load_train_eval_datasets_from_cfg
from .ema_utils import (
    EMAModel,
    EMASaveBothNoSwapCallback,
    PerplexityCallback,
    GranularityLoggingCallback,
    resume_ema_shadow_from_checkpoint,
)
from .trainer_matformer import MatFormerTrainer
...
def run_training_from_cfg(cfg: Dict):
    """
    Orchestrates training for MatFormer.
    Mirrors the HydraViT orchestrator (train_hydravit.py) in structure and behavior.
    """
    ...
    try:
        ...
        implementation = tr.get("implementation", "matformer_gemma")
        ...
        # MatFormer-specific params
        ffn_granularity_ratios = tr.get("ffn_granularity_ratios") or []
        ffn_granularity_weights = tr.get("ffn_granularity_weights")
        width_sampling_mode = (
            tr.get("width_sampling_mode", "global") or "global"
        ).lower()
        ...
        teacher_compile = bool(teacher_cfg.get("compile", False))
        ...
        # Seeding (mirrors HydraViT exactly)
        ...
        model = load_model(
            ...
            device=device_hint,
            ffn_granularity_ratios=ffn_granularity_ratios,
        )
        ...
        if teacher_enabled:
            ...
            try:
                ...
                pass
            # Hard-assert teacher weight tying
            assert _weights_tied(
                teacher_model
        ...
        # Validate granularity config
        if width_sampling_mode not in ("global", "per_layer"):
            raise ValueError(
                "width_sampling_mode must be either 'global' or 'per_layer'"
            )
        if not ffn_granularity_ratios:
            logger.warning(
                "ffn_granularity_ratios is empty; MatFormer will always run at full width."
            )
        if ffn_granularity_weights is not None:
            if len(ffn_granularity_weights) != len(ffn_granularity_ratios):
                raise ValueError(
                    "ffn_granularity_weights must match the length of ffn_granularity_ratios. "
                    f"Got {len(ffn_granularity_weights)} weights vs {len(ffn_granularity_ratios)} ratios."
                )
            s = sum(ffn_granularity_weights)
            if s <= 0:
                raise ValueError(
                    "ffn_granularity_weights must sum to a positive value."
                )
            ffn_granularity_weights = [w / s for w in ffn_granularity_weights]
        ...
        trainer = MatFormerTrainer(
            ...
            ffn_granularity_ratios=ffn_granularity_ratios,
            ffn_granularity_weights=ffn_granularity_weights,
            width_sampling_mode=width_sampling_mode,
            loss_type=loss_type,
        ...
        # Always add per-granularity logging
        trainer.add_callback(GranularityLoggingCallback(trainer))
        ...
        if ema_enabled and ema is not None and last_ckpt is not None:
            logger.info(
                f"[EMA] Attempting to resume EMA shadow from checkpoint: {last_ckpt}"
            )
            resume_ema_shadow_from_checkpoint(
                model=model,
                ema=ema,
        ...
        # Final save (mirrors HydraViT: model + state + tokenizer)
